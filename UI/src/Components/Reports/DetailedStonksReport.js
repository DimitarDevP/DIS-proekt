...
export default props => {
    ...
    const overloadedFields = [
        (<button onClick={e => {
            e.preventDefault()
            const newData = { ...exportData, data: [["Detailed Stocks Report For: ", getDateFromRange(new Date()), "", "Plastfix Shop System"], ...exportData.data.filter(f => f !== undefined)] }
            const file = new Blob([s2ab(generate(newData, filterOptions._export_columns))], { type: "application/octet-stream" })
            saveAs(file, 'Detailed Report.xlsx')
        }}>Export</button>)
    ]
    ...
    return (
        <Report
            ...
            groupByIdx={0}
            url={CONSTANTS.API_URL + "/api/reports/detailed_stocks"}
            validate={validate}
            ...
            name="Detailed Stocks Report"
            description="Filter and generate detailed stocks reports"
        />
    )
}