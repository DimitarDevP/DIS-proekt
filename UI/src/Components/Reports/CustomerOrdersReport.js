...
export default props => {
    ...
    const overloadedFields = [
        (<button onClick={e => {
            e.preventDefault()
            const newData = { ...exportData, data: [["Customer Orders Report For: ", getDateFromRange(filterOptions._date_range), "", "Plastfix Shop System"], ...exportData.data.filter(f => f !== undefined)] }
            const file = new Blob([s2ab(generate(newData, filterOptions._export_columns))], { type: "application/octet-stream" })
            saveAs(file, 'Customer Orders Report.xlsx')
        }}>Export</button>)
    ]
    ...
    const fields = [
        ...
        },
        {
            ...
            formType: "multiple",
            options: [...general._instances, {_id: 4, _name: "Vendor"}],
            display: "_name",
        },
        {
            name: "_status_id",
    ...
    return (
        <Report
            ...
            groupBy="State"
            url={CONSTANTS.API_URL + "/api/reports/orders_report"}
            validate={validate}
            ...
            description="Filter and generate customer orders reports"
            path="/orders/process/"
